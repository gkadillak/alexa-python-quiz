...
class GameTests(test_foundation.TestFoundation):
    ...
    def test_correct_answer(self):
        ...
        current_game_id = game.create_game(
            num_questions=1, session_id="123", user_id="1234"
        )
        current_game = session.query(models.Game).get(current_game_id)
        is_correct = game.answer_current_question(
            session_id=current_game.session_id, guess="1"
        )
...
class IntegrationTest(test_foundation.TestFoundation):
    ...
    def test_full_quiz(self):
        ...
        game.create_game(
            num_questions=2, session_id=requests.SESSION_ID, user_id=requests.USER_ID
        )
        with mock.patch("python_quiz.main.game.create_game"):
            self.test_app.post(
                "/python_quiz", data=json.dumps(requests.start_game_body)
            )
