...
class Game(base.BaseModel):
  ...
  """
    ...
    question_ids_snapshot = app.db.Column(
        postgresql.ARRAY(app.db.Integer), nullable=False
    )
    """Immutable list representing the quiz as instantiated"""

    question_ids = app.db.Column(
        MutableList.as_mutable(postgresql.ARRAY(app.db.Integer))
    )
    ...
    __mapper_args__ = {"polymorphic_identity": "games", "concrete": True}
